...
export function Map({ 
  ...
  // Helper to update map game state
  ...
  useEffect(() => {
    ...
    if (mapGameState.selectedAbility && mapGameState.selectedPiece === clientId) {
      return
    }
  ...
  }, [clientCoordinates, clientId, mapGameState.selectedAbility, mapGameState.selectedPiece])
    ...
    if (pieceClientId === clientId) {
      ...
      if (mapGameState.selectedAbility && mapGameState.selectedPiece === clientId) {
        ...
        if (targetPosition && attackerPosition) {
          ...
          const currentRangeSquares = calculateRangeSquares(attackerPosition, mapGameState.selectedAbility.ability.range)
  ...
  const handleAbilitySelect = (ability: AttackAbility, type: 'melee' | 'ranged') => {
    ...
    if (mapGameState.selectedAbility?.ability.attackName === ability.attackName && mapGameState.selectedAbility?.type === type) {
      updateMapGameState({
        selectedAbility: null,
        rangeSquares: [],
      });
    ...
    if (mapGameState.selectedPiece !== clientId) {
      updateMapGameState({ selectedPiece: clientId });
    }
    
    updateMapGameState({ selectedAbility: { ability, type } });
    ...
    <div className="hidden sm:block lg:mt-[2%] mt-[5%] w-full">
      <div className="flex gap-4 items-start justify-center w-full max-w-full">
        ...
        <div className="flex-shrink-0 w-64 bg-stone-900/90 border-2 border-stone-700 rounded-[16px] p-4 backdrop-blur-sm shadow-xl">
          ...
          {characterCreated && (
            <div className={`mb-4 ${mapGameState.selectedPiece === clientId ? 'ring-2 ring-yellow-400 rounded-lg p-2' : ''}`}>
                  ...
                  return allAbilities.length > 0 ? (
                    <>
                      {allAbilities.map((item, index) => {
                        const isSelected = mapGameState.selectedAbility?.ability.attackName === item.ability.attackName && 
            ...
            >
              <div className="grid grid-cols-10 h-full w-full" style={{ gap: '0' }}>
                {Array.from({ length: BOARD_SIZE * BOARD_SIZE }, (_, index) => {
                  ...
                  const isInRange = mapGameState.rangeSquares.some(
                    ...
                    >
                      {pieceClientId && (
                        <ChessPiece 
                          ...
                          position={position}
                          isSelected={mapGameState.selectedPiece === pieceClientId}
                          onSelect={() => handlePieceSelect(pieceClientId!)}
                          isDraggingDisabled={pieceClientId === clientId && mapGameState.selectedAbility !== null}
                        />
                      )}
                    </ChessSquare>
                  )
